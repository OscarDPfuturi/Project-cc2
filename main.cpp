...
Bala::Bala(int _x,int _y,int _dx, int _dy) : Nave(_x,_y){
    dx = _dx;
    dy = _dy;
}
...
void Item::mostrar(BITMAP* item,BITMAP* buffer){
...
/*void contacto_balas_naves(int &nro_disparos,const int cont,Bala *bala,Nave *nave,BITMAP *balai,BITMAP *buffer){
    if (contacto_balas_borde(bala)){
    ...
    if (bala[cont].contacto(nave)){
        nave->setVida(nave->getVida()-1);

        if (nave->getVida()==0) {
            srand(time(NULL));

            nave->setX(rand()%(ANCHO-50));
            nave->setY((-1)*(rand()%50));
...
int main()
{
    ...
    while (salida == 0){
        ...
        if(iniciar_juego(titulo,buffer,cursor,fondo_menu2,fondo_menu)){
            ...
            while(!key[KEY_ESC] && (nave_jugador->getVida() > 0) && nro_enemigos > 0){
                ...
                if(nro_disparos > 0){
                    for(int cont = 0; cont < nro_disparos; cont++){
                        ...
                        //contacto_balas_naves(nro_disparos,cont,bala_jugador[cont],nave_enemigo,bala1,buffer);
